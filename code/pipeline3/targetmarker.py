...
class Targetmarker:
    ...
    def __init__(self,frame,frame_id,model,bbox,target_region_cv2,bbox_format="tf",targetmarker_class_id=1,confirm_delta_thresh=15,num_bnn_samples=15,bnn_resize_size=16,output_folder="opf",video_id=1,initial_rcnn_score=0,minimum_type_decision_steps_threshold=5,maximum_type_decision_steps_threshold=30,rcnn_decision_threshold=0.8):
        ...
        if (targetmarker_class_id == 0 or targetmarker_class_id == 1) and initial_rcnn_score > self.rcnn_decision_threshold: 
    ...
    def __update_model_pred_mxtdot(self, image, frame_id):
        ...
        if not (self.is_type_decided or self.is_decided_by_rcnn):
            ...
            if self.targetmarker_class_id != 12:
                ...
                probabilities = [(len(predictions)-sum(predictions))/len(predictions),sum(predictions)/len(predictions)]
                self.bnn_predictions.append(probabilities)
                
                self.__plot_bnn_predictions(image,probabilities,predictions,frame_id)
                
                self.type_decision_counter += 1
                self.type_decision_lock_counter += 1
                
                predictions_avg = np.average(self.bnn_predictions,axis=0)
                predictions_max_index = predictions_avg.argmax()
                #logging.info("self.type_decision_counter: {}       predictions_avg: {}   predictions_max_index: {}".format(self.type_decision_counter,predictions_avg,predictions_max_index))
    ...
    def change_bounding_box(self,frame,bboxcv2,score,frame_id,label_id):
        ...
        if score > self.rcnn_decision_threshold: # immediately change marker_label based on rcnn score
            self.__reset_bnn_params()
            self.is_type_decided = True
            self.is_decided_by_rcnn = True
