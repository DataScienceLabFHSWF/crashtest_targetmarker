...
"""            
def get_new_detections(frame,frame_id,tm_changebbox,target_result_bboxes,rcnn_scores,label_id,frame_width,frame_height,all_targetmarker_list,active_targetmarker_list):
    ...
    for detection_id,bbox in enumerate(target_result_bboxes):
        ...
        #
        # create new marker as it is unknown yet 
...
marker_detector = htools.TFMarkerDetectorv3(join(model_folder,"targetmarker"), join(join(model_folder,"targetmarker"),"checkpoint"), 'pipeline')
    ...
    for video_id,video_title_ext in enumerate(video_files):
        ...
        if vid_ops.load_video(filename_in):
            ...
            while get_frame_succ:
                ...
                mxt_succs, target_result_bboxes_mxt, scores_mxt, dot_succs, target_result_bboxes_dot, scores_dot = marker_detector.detect(current_frame_normalized,min_detection_score_mxt=rcnn_mxt_threshold,min_detection_score_dot=rcnn_dot_threshold)
                
                mxt_new_det_succ, no_duplicate_bboxes_list_mxt, active_targetmarker_list, all_targetmarker_list = get_new_detections(current_frame_normalized,frame_id,tm_changebox,target_result_bboxes_mxt,scores_mxt,mxtlabel,frame_width,frame_height,all_targetmarker_list,active_targetmarker_list)
                dot_new_det_succ, no_duplicate_bboxes_list_dot, active_targetmarker_list, all_targetmarker_list = get_new_detections(current_frame_normalized,frame_id,tm_changebox,target_result_bboxes_dot,scores_dot,dotlabel,frame_width,frame_height,all_targetmarker_list,active_targetmarker_list)
