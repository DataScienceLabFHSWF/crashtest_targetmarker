...
#def get_new_detections(frame,target_result_bboxes,rcnn_scores,label_id,frame_width,frame_height,all_targetmarker_list,active_targetmarker_list,current_active_targetmarker_list):
#    print("enter check_new_detections    label_id: {}    len(target_result_bboxes): {}".format(label_id,target_result_bboxes))
#    print("rcnn_scores: {}".format(rcnn_scores))
...
#            label_id_skip = 12
#            temp_marker = Targetmarker(current_frame_normalized, frame_id, mxtdot_model, bboxcv2, target_region, "cv2", label_id_skip, confirm_delta_thresh=tm_confirm_delta_thresh, num_bnn_samples=tm_num_bnn_forward_passes, bnn_resize_size=tm_bnn_resize_size, output_folder=output_mxtdot_plot_folder,video_id=video_id,initial_rcnn_score=rcnn_score,minimum_type_decision_steps_threshold=tm_minimum_type_decision_steps_threshold,maximum_type_decision_steps_threshold=tm_maximum_type_decision_steps_threshold)
...
#            current_active_targetmarker_list.append(temp_marker)
        ...
        print("detection score: {}".format(rcnn_score))
        #
        ...
        for tm in current_active_targetmarker_list:
            ...
            if htools.calc_iou_cv2(bboxcv2,tm.bboxes_cv2[-1],duplicate_iou_threshold)[0]:
                if rcnn_score > tm.rcnn_scores[-1]: 
                    tm_changebbox.append([frame_id,tm.rcnn_scores[-1],rcnn_score])
                    print("tm.id: {}    bboxcv2: {}    tm.start_frame_id: {}".format(tm.marker_id,bboxcv2,tm.start_frame_id))
                    tm.change_bounding_box(frame,bboxcv2,rcnn_score,frame_id,label_id)
                    
                tm.confirm()
#                print("confirm: tm.id: {}    bboxcv2: {}    tm.start_frame_id: {}".format(tm.marker_id,bboxcv2,tm.start_frame_id))
...
tracker_iou_threshold_quad = 0.3
...
#mxtdot_model = tfk.models.load_model(join(join(model_folder,"mxtdot/"),"saved_model"))
##car_detector = htools.TFCarDetector(join(model_folder,"car"), join(join(model_folder,"car"),"checkpoint"), join(join(model_folder,"car"),"testlabels_mscoco.pbtxt"), 'faster_rcnn_resnet101_v1_800x1333_coco17_gpu-8')
#marker_detector = htools.TFMarkerDetectorv3(join(model_folder,"targetmarker"), join(join(model_folder,"targetmarker"),"checkpoint"), join(join(model_folder,"targetmarker"),"testlabels_mscoco.pbtxt"), 'pipeline')
#output_mxtdot_plot_folder = join(output_directory,"bnn_plots")
...
for input_directory in input_directories:
    ...
    for video_id,video_title_ext in enumerate(video_files):
        ...
        filename_out = join(output_directory,video_title_ext)
        ...
        if vid_ops.load_video(filename_in):
            ...
            print("length: {} frames\n".format(vid_ops.length_in_frames))
            ...
            # get_frame_succ, current_frame, current_frame_normalized = vid_ops.get_frame_at_pos(410)
...
#                image_slice_target_region = current_frame_normalized[target_region[1]:(target_region[1]+target_region[3]-1),target_region[0]:(target_region[0]+target_region[2]-1)]
#                scale_slice_target_region = current_frame_normalized[scale_target_region[1]:(scale_target_region[1]+scale_target_region[3]-1),scale_target_region[0]:(scale_target_region[0]+scale_target_region[2]-1)]

#                cv.imwrite(join(output_directory,"image_slice.jpg"),image_slice_target_region)
#                cv.imwrite(join(output_directory,"scale_slice.jpg"),scale_slice_target_region)
#                cv.imwrite(join(join(cwd,"output"),"image_slice.jpg"),image_slice_target_region)
#                cv.imwrite(join(join(cwd,"output"),"scale_slice.jpg"),scale_slice_target_region)
                

                for targetmarker in active_targetmarker_list:
#                    print("before targetmarker.update   tm.is_active: {}".format(targetmarker.is_active))
                    targetmarker.update(current_frame,current_frame_normalized,frame_id)
#                    print("after targetmarker.update   tm.is_active: {}".format(targetmarker.is_active))
                    if targetmarker.is_active:
#                        print("targetmarker.is_active")
                        current_active_targetmarker_list.append(targetmarker)

#                print("len(current_active_targetmarker_list): {} ".format(len(current_active_targetmarker_list)))
#                mxtdot_succs, target_result_bboxes_mxtdot, scores_mxtdot, quad_succs, target_result_bboxes_quad, scores_quad,_ = marker_detector.detect(current_frame_normalized,min_detection_score_quad = 0.1)
...
#                current_frame = htools.draw_bounding_box(current_frame,scale_target_region,"slice_region_for_scale_detection",1)
                
#                if len(active_targetmarker_list) != 0:
#                    current_frame = htools.draw_bounding_box(current_frame,active_targetmarker_list[-1].target_region_bboxcv2,"slice_region_for_marker_detection",2)
...
#                    break
                ...
                if get_frame_succ:
                    print("len(active_targetmarker_list): {} ".format(len(active_targetmarker_list)))
                    frame_id += 1
